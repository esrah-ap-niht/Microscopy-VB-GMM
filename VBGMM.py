...
@ray.remote
def score_samples(indexes): 
    return DPGMM.score_samples( full_data[indexes,:] )
...
while run: 
    ...
    for i, layer in enumerate(image_list): 
      #  res = sum(map(lambda i: i == True, (np.sum(array, axis = 2) > eds_quality_threshold).flatten()))
      #  data = np.array( [  (random.sample(list(array[:, :, i][np.sum(array, axis = 2) > eds_quality_threshold].flatten() ), min(histogram_size, res) ) )  ,  [layer] *min(histogram_size, res) ]  ).T
    ...
    while autodidactic_loop == True: 
        ...
        available_memory = psutil.virtual_memory().free/(1073741824) # available memory in GB 
        scoring_memory_multiplier = 
        maximum_packet_size = available_memory / (scoring_memory_multiplier * num_cpus)
        packet_size = min( *scoring_memory_multiplier, maximum_packet_size)
        
        
        sys.getsizeof(full_data)*scoring_memory_multiplier/1073741824
        
        
        ray_data = ray.put(full_data)
        results = ( ray.get( [ score_samples.remote( ray_data, [images[j] for j in range(len(images) )][k] ) for k in range(len(images))]) )
